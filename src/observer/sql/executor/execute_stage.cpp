...
RC ExecuteStage::handle_request(SQLStageEvent* sql_event) {
    RC rc = RC::SUCCESS;
    // 在Transformer和Optimizer阶段，对于查询类型的SQL会将Stmt转换成LogicalOperator，优化后输出PhysicalOperator
    const unique_ptr<PhysicalOperator>& physical_operator = sql_event->physical_operator();
    if (physical_operator != nullptr) {
        // 将PhysicalOperator(物理执行计划)转换为SqlResult(执行结果)
    ...
    if (stmt != nullptr) {
        // 如果是命令执行类型的SQL请求，会创建对应的 CommandExecutor
        CommandExecutor command_executor;
        // 将CommandExecutor执行后通过SqlResult输出结果
