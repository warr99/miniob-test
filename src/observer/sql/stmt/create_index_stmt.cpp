...
RC CreateIndexStmt::create(Db* db, const CreateIndexSqlNode& create_index, Stmt*& stmt) {
    ...
    if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) || create_index.attribute_names.size() == 0 || is_blank(create_index.attribute_names[0].c_str())) {
    ...
    stmt = new CreateIndexStmt(table, field_metas, create_index.index_name, stringToIndex(create_index.index_type));
    return RC::SUCCESS;
}
